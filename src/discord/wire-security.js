...
// Mass-guard + snapshots + READ-ONLY restore preview posts.
...
async function maybeEnforceMassGuard({guild,executor,total,threshold,reason,enforceFlag,logToChannel}){
  const actuallyEnforce=!!enforceFlag;
  if(actuallyEnforce){
    await enforceSafely({ guild, actor: executor, action: "massGuard", reason,
      onEnforce: async () => {
        const member = await guild.members.fetch(executor.id).catch(() => null);
        if (member?.moderatable && member.timeout) { await member.timeout(60_000,"MassGuard: destructive activity threshold reached"); return "timeout(60s)"; }
        return "not-moderatable";
      }
    });
  } else {
    log.warn({guild:guild.id,user:executor.id,total},"DRY-RUN: would enforce massGuard");
  }
...
export async function wireSecurity(client){
  client.once("clientReady", async () => {
    ...
    for (const [, g] of client.guilds.cache) {
      ...
      log.info({ windowMs, enforcing, logToChannel: logToChan, guild: `[id:${g.id}]`, perms }, "security wiring active");
      if (!perms.viewAudit) log.warn({ guild: g.id }, "missing View Audit Log");
    ...
    try {
      ...
      if (executor) {
        ...
        if (total >= merged.threshold) {
          await maybeEnforceMassGuard({ guild, executor, total, threshold: merged.threshold,
            reason:`channelDelete; score ${total.toFixed(2)} >= ${merged.threshold}`,
            enforceFlag:getEnforceFlag(ff), logToChannel:getLogToChannelFlag(ff) });
